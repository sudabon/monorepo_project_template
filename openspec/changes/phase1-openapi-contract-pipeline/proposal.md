...
- `packages/api-client` に TanStack Query の `queryOptions` を返す薄いラッパを手書きで追加する。生成物の上に 1 枚被せる形とし、生成物そのものは編集しない
...
- **依存追加**: `oapi-codegen`（Go、ツール依存）、`openapi-typescript`（TS、dev 依存）、`openapi-fetch`（TS、実行時依存）、`@tanstack/react-query`（`queryOptions` の型のため peer として扱う）、契約 lint ツール
- **後続への影響**: Phase 2 は生成された Echo サーバインタフェースを実装する。Phase 4/5 は `packages/api-client` の `queryOptions` を使う
