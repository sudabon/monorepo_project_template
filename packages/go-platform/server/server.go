...
func ShutdownTimeout() (time.Duration, error) {
	// ECS defaults stopTimeout to 30 seconds. Keep this 20-second drain below
	// stopTimeout, leaving time to close pools and flush logs before SIGKILL.
	// If overridden, SHUTDOWN_TIMEOUT must remain below the task's stopTimeout.
	wait := 20 * time.Second
...
func Serve(listener net.Listener, handler http.Handler, wait time.Duration) error {
	...
	// A fresh context lets requests finish after the signal context is cancelled.
